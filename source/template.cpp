...
template<typename T, typename U> $f::$o& operator<<($f::$o& os, const std::pair<T,U>& p) { return os << p.first << ' ' << p.second; }
template<typename... Args> $f::$o& operator<<($f::$o& os, const std::tuple<Args...>& t) { return apply([&](auto&&... args){ ((os << args << ' '), ...); }, t), os; }
template<typename T> $f::$o& operator<<($f::$o& os, const std::vector<T>& v) { for (auto& e : v) os << e << ' '; return os; }

template<typename T, typename U> $f::$i& operator>>($f::$i& is, std::pair<T,U>& p) { return is >> p.first >> p.second; }
template<typename... Args> $f::$i& operator>>($f::$i& is, std::tuple<Args...>& t) { return apply([&](auto&&... args){ ((is >> args), ...); }, t), is; }
template<typename T> $f::$i& operator>>($f::$i& is, std::complex<T>& v) { T x; is >> x; v.real(x); is >> x; v.imag(x); return is; }
template<typename T> $f::$i& operator>>($f::$i& is, std::vector<T>& v) { for (auto& e : v) is >> e; return is; }

template<typename T> $f::$o& operator,($f::$o& os, const T& x) { return os << x << ' '; }
