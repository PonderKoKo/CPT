 ...
 */
void dfs(num source, const Graph& adj, vector<bool>& visited) {
    // Do something
    visited[source] = true;
    for (num v : adj[source]) {
        if (!visited[v])
            dfs(v, adj, visited);
    }
}
...
// Unrooted, undirected
void treeDFS(num node, num parent, const Graph& adj) {
    // Pre-order
    for (num v : adj[node]) {
        if (v != parent) {
            treeDFS(v, node, adj);
...
// Rooted and directed
void treeDFS(num node, const Graph& children) {
    ...
    while (!q.empty()) {
        ...
        for (const auto& [to, weight] : adj[v])
            if (distance[v] + weight < distance[to])
                q.emplace(distance[to] = distance[v] + weight, to); // You would also update the parent in this if
    }
    return distance;
}

// Thorup's algorithm seems interesting


vector<num> topo_sort (const Graph& adj) {
    vector<num> in_degree(adj.size(), 0);
